...
export default async function handler(req, res) {
  ...
  try {
    ...
    const scriptText = meta.script || meta.notes || "";
    if (!frames?.length && !["text", "audio"].includes(creativeFormat)) return res.status(400).json({ error: "No frames" });
...
{"creative_format":"${creativeFormat}","headline_verdict":"One powerful CMO-ready sentence","overall_grade":"A+|A|A-|B+|B|B-|C+|C-|D|F","grade_note":"One line","creative_summary":"3-4 sentence summary","viral_potential":INT,"hook_strength":INT,"hold_rate":INT,"emotional_peak":INT,"brand_recall":INT,"memory_encoding":INT,"sound_off_survival":INT,"share_intent":INT,"creative_efficiency":INT,"ad_fatigue_risk":INT,"cultural_resonance":INT,"celebrity_talent_index":INT,"brand_safety":INT,"regulatory_compliance":INT,"first_party_data_opportunity":INT,"carbon_signal":INT,"system1_vs_system2":INT,${formatSpecificSchema},"attention_curve":[INT,INT,INT,INT,INT,INT,INT,INT,INT,INT],"emotion_curve":[INT,INT,INT,INT,INT,INT,INT,INT,INT,INT],"emotion_types":{"joy":[INT,INT,INT,INT,INT],"surprise":[INT,INT,INT,INT,INT],"trust":[INT,INT,INT,INT,INT],"fear":[INT,INT,INT,INT,INT],"desire":[INT,INT,INT,INT,INT],"curiosity":[INT,INT,INT,INT,INT]},"brain_regions":{"visual_cortex":INT,"prefrontal_cortex":INT,"amygdala":INT,"hippocampus":INT,"auditory_cortex":INT,"mirror_neurons":INT,"nucleus_accumbens":INT,"anterior_cingulate":INT},"cognitive_channels":{"visual":INT,"auditory":INT,"motion":INT,"text_overlay":INT,"brand_elements":INT,"human_faces":INT,"color_saturation":INT},"platform_scores":{"youtube_preroll_6s":INT,"youtube_preroll_15s":INT,"youtube_instream":INT,"instagram_reels":INT,"instagram_stories":INT,"instagram_feed":INT,"meta_feed":INT,"meta_stories":INT,"tiktok":INT,"linkedin_feed":INT,"twitter_x":INT,"tv_broadcast":INT,"ctv_ott":INT,"dooh":INT,"programmatic_display":INT},"competitive_context":{"category_avg_viral":INT,"category_avg_hook":INT,"category_avg_hold":INT,"category_avg_recall":INT,"position":"above_average|average|below_average|category_leader","benchmark_note":"One sentence"},"sound_analysis":{"sound_dependency":INT,"music_effectiveness":INT,"voiceover_clarity":INT,"sound_off_text_quality":INT,"asmr_trigger":INT,"sonic_branding":INT,"sound_note":"2-3 sentences"},"privacy_and_data_audit":{"data_collection_present":false,"consent_mechanism_visible":false,"qr_code_present":false,"url_cta_present":false,"hashtag_present":false,"regulatory_disclaimers_visible":false,"dpdp_compliance_risk":"low|medium|high","privacy_note":"2-3 sentences"},"scenes":[{"ts":"0:00","name":"Format-appropriate scene, zone, copy section, or audio moment","desc":"1-2 sentence format-aware description","attention":INT,"emotion":INT,"system_mode":"S1|S2|TRANSITION","cognitive_load":INT,"risk_flag":"none|drop_zone|ad_avoidance|low_attention|clutter|unclear_message","drop_second":INT|null,"badges":["memorable"]},{"ts":"0:01","name":"Second zone/section/moment","desc":"1-2 sentence description","attention":INT,"emotion":INT,"system_mode":"S1|S2|TRANSITION","cognitive_load":INT,"risk_flag":"none|drop_zone|ad_avoidance|low_attention|clutter|unclear_message","drop_second":INT|null,"badges":["brand_moment"]}],"strategic_insights":[{"num":1,"title":"Insight Title","body":"2-3 sentence insight referencing specific metric scores and the creative format","verdict":"WIN|RISK|TIP|WATCH","vtype":"brand|creative|media|audience"}],"cmo_actions":[{"num":1,"title":"Action Title","body":"2-3 sentence format-appropriate recommendation with expected lift and timeline","priority":"P1|P2|P3","impact":"high|medium|low","effort":"easy|medium|hard","estimated_uplift_pct":INT}]}`;
    ...
    const resp = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 8000,
        system: prompt,
        messages: [{ role: "user", content: [...imageContent, { type: "text", text: `Score this ${meta.brand || "creative"} ad using the ${creativeFormat} format rules. Return ONLY JSON.` }] }]
      })
    });
    if (!resp.ok) {
