...
OUTCOME EXPLAINABILITY — return an outcome_explainability object.
For every forecast KPI, explain why the score was produced and what a CMO or creative lead should do next. Each KPI object must include:
- positive_drivers: 1-3 short, metric-backed reasons that support the score.
- negative_drivers: 1-3 short, metric-backed risks that suppress the score.
- format_drivers: one format-specific explanation. Static image uses stopping power, visual hierarchy, brand prominence, message clarity, and CTA clarity. Video uses hook, hold, pacing, attention recovery, and completion signals. Motion/GIF uses first-frame strength, loop clarity, motion salience, message persistence, and loop fatigue. Audio uses voice clarity, sonic branding, mnemonic strength, CTA recall, and real-world audibility. Text/script uses headline strength, proposition clarity, persuasion, memorability, readability, and claim risk.
- platform_modifier: one sentence naming the best or weakest platform context from platform_outcome_forecast.
- improve_action: one practical creative revision to improve the score. Do not give a media-only action unless the KPI is media_dependency.
Also return outcome_explainability.platform_modifiers keyed by the same platform keys as platform_outcome_forecast. These are explanatory summaries only; do not create new platform scores.
Do not mention VTR, completion, seconds, or video timelines for static_image or text formats. Use "view-through attention fit" or "first-glance retention" when a non-video format needs an attention proxy.
