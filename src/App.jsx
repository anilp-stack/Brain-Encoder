...
export default function App(){
  ...
  const handleAnalyze=useCallback(async()=>{
    ...
    try{
      ...
      if(!AKEY) throw new Error("API key not configured. Add VITE_ANTHROPIC_API_KEY to Netlify environment variables.");

      const meta = {...form,...frameData};
      const isImage = frameData.isImage||false;
      const duration = frameData.duration||0;
      const usedFrames = frameData.frames.slice(0,3);
...
{"headline_verdict":"ONE powerful sentence a CMO will remember","overall_grade":"A+|A|A-|B+|B|B-|C+|C|C-|D|F","grade_note":"One line justification","creative_summary":"4-5 sentence executive summary","viral_potential":INT,"hook_strength":INT,"hold_rate":INT,"emotional_peak":INT,"brand_recall":INT,"memory_encoding":INT,"sound_off_survival":INT,"share_intent":INT,"creative_efficiency":INT,"ad_fatigue_risk":INT,"cultural_resonance":INT,"celebrity_talent_index":INT,"brand_safety":INT,"regulatory_compliance":INT,"first_party_data_opportunity":INT,"carbon_signal":INT,"system1_vs_system2":INT,"attention_curve":[INT,INT,INT,INT,INT,INT,INT,INT,INT,INT],"emotion_curve":[INT,INT,INT,INT,INT,INT,INT,INT,INT,INT],"emotion_types":{"joy":[INT,INT,INT,INT,INT],"surprise":[INT,INT,INT,INT,INT],"trust":[INT,INT,INT,INT,INT],"fear":[INT,INT,INT,INT,INT],"desire":[INT,INT,INT,INT,INT],"curiosity":[INT,INT,INT,INT,INT]},"brain_regions":{"visual_cortex":INT,"prefrontal_cortex":INT,"amygdala":INT,"hippocampus":INT,"auditory_cortex":INT,"mirror_neurons":INT,"nucleus_accumbens":INT,"anterior_cingulate":INT},"cognitive_channels":{"visual":INT,"auditory":INT,"motion":INT,"text_overlay":INT,"brand_elements":INT,"human_faces":INT,"color_saturation":INT},"platform_scores":{"youtube_preroll_6s":INT,"youtube_preroll_15s":INT,"youtube_instream":INT,"instagram_reels":INT,"instagram_stories":INT,"instagram_feed":INT,"meta_feed":INT,"meta_stories":INT,"tiktok":INT,"linkedin_feed":INT,"twitter_x":INT,"tv_broadcast":INT,"ctv_ott":INT,"dooh":INT,"programmatic_display":INT},"scenes":[{"ts":"0:00-0:06","name":"Scene Name","desc":"3-4 sentence neural analysis","attention":INT,"emotion":INT,"system_mode":"system1|system2|mixed","cognitive_load":"low|medium|high|overload","risk_flag":"none|drop_zone|ad_avoidance|cognitive_overload|pacing_issue","badges":["badge1","badge2"]}],"strategic_insights":[{"num":"01","title":"Insight Title","body":"4-6 sentence analysis referencing advertising science","verdict":"Actionable verdict","vtype":"risk|win|tip|watch"}],"cmo_actions":[{"num":"01","title":"Action Title","body":"3-4 sentence recommendation","priority":"critical|high|medium|low","impact":"Predicted metric improvement","effort":"easy|medium|hard"}],"competitive_context":{"category_avg_viral":INT,"category_avg_hook":INT,"category_avg_hold":INT,"category_avg_recall":INT,"position":"above_average|average|below_average|category_leader","benchmark_note":"One sentence benchmark"},"sound_analysis":{"sound_dependency":INT,"music_effectiveness":INT,"voiceover_clarity":INT,"sound_off_text_quality":INT,"asmr_trigger":INT,"sonic_branding":INT,"sound_note":"2-3 sentences on audio strategy"},"privacy_and_data_audit":{"data_collection_present":false,"consent_mechanism_visible":false,"qr_code_present":false,"url_cta_present":false,"hashtag_present":false,"regulatory_disclaimers_visible":false,"dpdp_compliance_risk":"low|medium|high","privacy_note":"2-3 sentences on data privacy"}}`;
      ...
      const resp = await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          ...
          messages:[{
            ...
            content:[
              ...imageContent,
              {type:"text",text:`Analyze these ${usedFrames.length} frames for ${meta.brand||"this brand"}. ${meta.notes?"Context: "+meta.notes:""} Return ONLY the JSON object.`}
            ]
          }]
        })
      });
      ...
      try{data=JSON.parse(respText);}catch(e){
        throw new Error("API error: "+respText.substring(0,120));
      }
      if(!resp.ok){
      ...
      try{analysis=JSON.parse(clean);}catch(e){
        throw new Error("Could not parse analysis JSON: "+clean.substring(0,100));
      }

      setProgress(100);setProgressMsg("Report ready.");
      await new Promise(r=>setTimeout(r,400));
      setResults(analysis);setStage("results");setTab("summary");
