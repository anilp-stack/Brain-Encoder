...
function Takeaway({icon,title,items,color}){var c=color||"#00d4ff";if(!items||items.length===0)return null;return(<div style={{background:"#1a1e2e",border:"1px solid "+c+"33",borderRadius:14,padding:28,marginTop:24,borderLeft:"4px solid "+c}}><div style={{fontSize:15,fontWeight:700,color:c,marginBottom:14,display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:20}}>{icon}</span>{title}</div>{items.map(function(item,i){return(<div key={i} style={{display:"flex",gap:12,marginBottom:i<items.length-1?12:0,alignItems:"flex-start"}}><span style={{color:c,fontWeight:800,fontSize:14,marginTop:2}}>{item.type==="do"?"✅":item.type==="fix"?"🔧":item.type==="warn"?"⚠️":"🏆"}</span><span style={{fontSize:14,color:"#94a3b8",lineHeight:1.7}}><b style={{color:"#e2e8f0"}}>{item.label}</b> {item.text}</span></div>)})}</div>);}

function getT(r){var t={};t.summary=[];if((r.viral_potential||0)>=70)t.summary.push({type:"win",label:"Strong viral potential.",text:"Prioritize for social-first distribution."});else t.summary.push({type:"fix",label:"Viral potential below 70.",text:"Add a pattern interrupt, emotional spike, or relatable hook."});if((r.hold_rate||0)<60)t.summary.push({type:"warn",label:"Hold rate critical.",text:(100-(r.hold_rate||0))+"% drop off. Re-edit weakest section."});if((r.sound_off_survival||0)<60)t.summary.push({type:"fix",label:"Sound-off survival low.",text:"Add kinetic text overlays. 80%+ social is sound-off."});if((r.brand_recall||0)>=80)t.summary.push({type:"win",label:"Excellent brand recall.",text:"Distinctive brand assets well-placed."});else t.summary.push({type:"fix",label:"Brand recall needs work.",text:"Increase logo visibility, add product shot earlier."});if((r.memory_encoding||0)<65)t.summary.push({type:"warn",label:"Risk: watched but forgotten.",text:"Add an emotional anchor moment."});t.neural=[];var br=r.brain_regions||{};if((br.amygdala||0)<50)t.neural.push({type:"fix",label:"Amygdala low.",text:"Not triggering emotion. Add surprise, humor, or vulnerability."});else t.neural.push({type:"win",label:"Strong emotional activation.",text:"Amygdala engaged for memory encoding."});if((br.prefrontal_cortex||0)>70&&(br.amygdala||0)<60)t.neural.push({type:"warn",label:"Over-indexing rational.",text:"Thinking not feeling. Lead with emotion."});if((br.mirror_neurons||0)>=65)t.neural.push({type:"win",label:"Mirror neurons active.",text:"Viewers empathizing. Drives share intent."});else t.neural.push({type:"fix",label:"Low mirror neurons.",text:"Add facial expressions, human interaction."});var s1=r.system1_vs_system2||50;if(s1>=65&&s1<=75)t.neural.push({type:"win",label:"System 1/2 optimal.",text:"Leads with emotion, layers rational."});else if(s1<65)t.neural.push({type:"fix",label:"Over-indexing System 2.",text:"Reduce claims. Lead with feeling."});else t.neural.push({type:"warn",label:"Over-indexing System 1.",text:"Add one clear product claim."});t.attention=[];var at=r.attention_curve||[];if(at.length>0){var pk=Math.max.apply(null,at);var lo=Math.min.apply(null,at);t.attention.push({type:"do",label:"Peak at ~"+at.indexOf(pk)+"s ("+pk+"%).",text:"Use as thumbnail for static placements."});if(lo<50)t.attention.push({type:"warn",label:"Drops to "+lo+"% at ~"+at.indexOf(lo)+"s.",text:"Your drop zone. Cut or add visual interrupt."});var av=Math.round(at.reduce(function(a,b){return a+b},0)/at.length);t.attention.push({type:av>=65?"win":"warn",label:"Avg attention: "+av+"%.",text:av>=65?"Above 60% recall threshold.":"Below 60%. Consider shorter cut."});}else{t.attention.push({type:"do",label:"Review heatmap above.",text:"Green=strong. Red=needs interrupts."});}t.emotion=[];if((r.emotional_peak||0)>=70)t.emotion.push({type:"win",label:"Strong emotional peak.",text:"Triggers genuine emotion for recall."});else t.emotion.push({type:"fix",label:"Moderate emotional peak.",text:"Add surprise, joy, or vulnerability."});if((r.share_intent||0)>=65)t.emotion.push({type:"win",label:"High share intent.",text:"Optimize: hashtags, under 30s, self-explanatory first frame."});else t.emotion.push({type:"fix",label:"Share intent below 65.",text:"Add identity signaling or social currency."});t.scenes=[];t.scenes.push({type:"do",label:"Use scene data for re-edits.",text:"Keep scenes above 70. Rework scenes below 50."});t.scenes.push({type:"do",label:"Check System mode transitions.",text:"Frequent S1-S2 shifts cause cognitive fatigue."});t.scenes.push({type:"do",label:"Prioritize drop zones.",text:"Drop zone scenes = where viewers leave. #1 re-edit priority."});t.platforms=[];var ps=r.platform_scores||{};var be="";var bv=0;var wo="";var wv=100;for(var k in ps){if(ps[k]>bv){bv=ps[k];be=k;}if(ps[k]<wv){wv=ps[k];wo=k;}}if(be)t.platforms.push({type:"win",label:"Best: "+be.replace(/_/g," ")+" ("+bv+").",text:"Prioritize media spend here."});if(wo)t.platforms.push({type:"warn",label:"Weakest: "+wo.replace(/_/g," ")+" ("+wv+").",text:"Do not run here without format-specific re-edit."});t.sound=[];var sn=r.sound_analysis||{};if((sn.sound_dependency||0)>70)t.sound.push({type:"warn",label:"High sound dependency.",text:"On social (80%+ sound-off) will underperform. Create kinetic text variant."});else t.sound.push({type:"win",label:"Low sound dependency.",text:"Works without audio. Ready for sound-off."});if((r.sound_off_survival||0)<50)t.sound.push({type:"fix",label:"Sound-off survival critical.",text:"Add bold text overlays and high-contrast subtitles."});t.privacy=[];var pr=r.privacy_and_data_audit||{};if(!pr.url_cta_present&&!pr.qr_code_present&&!pr.hashtag_present)t.privacy.push({type:"fix",label:"No conversion path.",text:"No QR/URL/hashtag. Add a search CTA."});if(pr.dpdp_compliance_risk==="high")t.privacy.push({type:"warn",label:"High DPDP risk.",text:"Review with legal before programmatic."});else t.privacy.push({type:"win",label:"DPDP: "+(pr.dpdp_compliance_risk||"low")+".",text:"No major flags."});if(!pr.regulatory_disclaimers_visible)t.privacy.push({type:"fix",label:"No disclaimers visible.",text:"Add if making health/financial claims."});return t;}

function TabTakeaway({tab,r}){if(!r)return null;var t=getT(r);var m={"summary":["📋","What This Means for You","#d4a017"],"neural":["🧠","Neural Map — What to Do","#a855f7"],"attention":["👁","Attention Economics — Actions","#f59e0b"],"emotion":["❤️","Emotional Architecture — Actions","#ec4899"],"scenes":["🎬","Scene Intelligence — How to Use This","#14b8a6"],"platforms":["📱","Platform Strategy — Where to Run This","#3b82f6"],"sound":["🔊","Sound Strategy — Actions","#a855f7"],"privacy":["🛡️","Privacy & Compliance — Actions","#f59e0b"]};var c=m[tab];if(!c||!t[tab])return null;return <Takeaway icon={c[0]} title={c[1]} color={c[2]} items={t[tab]}/>;}
